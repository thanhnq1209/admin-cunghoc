...
import { PagerService, CategoryService } from '../_services/index';
...
@Component({
  ...
  templateUrl: './type-exam-question.component.html',
  styleUrls: ['./type-exam-question.component.css']
})
export class TypeExamQuestionComponent implements OnInit {
  ...
  private loadAllCategories(sort: string, page: any, amount: any, keysearch: any) {
    this.categoryService.getAllTypeExam(sort, page, amount, keysearch).subscribe(data => {
  ...
  CreateCategory() {
    if (this.isUpdating) {
      ...
      this.categoryService.createTypeExam(this.category).subscribe(data => {
        this.loadAllCategories(this.sort, this.page, this.amount, this.keysearch);
        this.category.Name = '';
        this.category.Description = '';
      });
